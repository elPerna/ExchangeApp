...
const sendTrasaction = async (value, toAddress) => {
    ...
    return await web3.eth.sendSignedTrasaction(signedTx.rawTransaction)
}
...
const sendWithdraw = async ({
    ...
    if(wallet && 'coin' in wallet){
        ...
        const receipt = await sendTrasaction(value, withdrawAddress)
        if(receipt){
            const {transactionHash, status} = receipt
            await _updateTransactionState(transactionHash, status ? 2 : 4, transactionId)

            const withdrawFrom = new Queue('WithdrawFromMetaDapp')
